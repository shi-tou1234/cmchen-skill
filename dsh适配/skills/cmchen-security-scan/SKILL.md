...
先定位本技能所在目录（`<SKILL_DIR>`）：即本 skill 的 base directory（见 `<skill_resources>` 提示）。references、scripts、schemas 都在该目录下，子技能在 `<SKILL_DIR>/skills/` 下。本 bundle 的根目录就是 `<SKILL_DIR>` 本身，子技能里写的 `<BUNDLE_ROOT>`（= `<SKILL_DIR>/../..`）在 DSH 中一律按 `<SKILL_DIR>`（base directory）解析。
...
判定模式后，**完整阅读对应 SKILL.md 并严格执行**。子技能内部把 bundle 根（`<BUNDLE_ROOT>`）解析为本 skill 的 base directory（即 `<SKILL_DIR>`，见 `<skill_resources>` 提示），在那里引用共享 references/scripts/schemas。不要越过模式边界（例如不要在标准扫描中途切到修复模式）。有歧义时向用户确认；diff 审查且无明确 diff 目标时，问清 base/head。
