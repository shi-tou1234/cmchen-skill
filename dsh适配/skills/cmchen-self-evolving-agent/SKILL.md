...
本 SKILL.md 所在目录即为 `$SKILL_DIR`（即本 skill 的 base directory，见 `<skill_resources>` 提示），所有路径基于此目录。
首次使用时创建 `memory/` 骨架结构并初始化。注意：`memory/` 是运行时数据，若 skill 安装在只读位置（如 node_modules），请把 `$SKILL_DIR/memory` 的实际读写位置迁移到一个可写目录（如 `%USERPROFILE%\.dsh\cmchen-self-evolving-agent\memory`），并在本会话中始终使用该路径。
